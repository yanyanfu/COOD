...
for i in range(30000):
    s = round(random.gauss(11.341275514256214, 8.165116282526235))
...
for data in sampled_dataset:
    dt2[len(data['code_tokens'])].append(data)


sampling2 = {}
for i in range(15000):
    s = round(random.gauss(98.05638154237154, 82.55078550597402))
    if s not in sampling2.keys():
        sampling2[s] = 1
    else:
        sampling2[s] += 1

sampled_dataset2 =[]
for key, value in sampling2.items():
    if key in dt2.keys():
        m = min(value, len(dt2[key]))
        sampled_dataset2.extend(dt2[key][:m])
print(len(sampled_dataset2))
